    ...
    select
      (select jsonb_build_object(
       ...
       where upper(trim(sub_bidang)) = 'HARGI'
         and (upt = any(${selUnits}) or ${selUnits.length === 0})) as ce,
      ...
      (select coalesce(jsonb_agg(x order by x.total desc), '[]'::jsonb) from (
         select gardu,
                ...
                count(*)::int as total,
                count(*) filter (
                  where trim(tahun) = extract(year from now()::timestamptz at time zone 'Asia/Jakarta')::int::text
                )::int as total_year,
                count(*) filter (
                  where trim(tahun) = extract(year from now()::timestamptz at time zone 'Asia/Jakarta')::int::text
                    ...
                    and upper(trim(bulan)) in (
                      (array['JANUARY','FEBRUARY','MARCH','APRIL','MAY','JUNE',
                        ...
                        [extract(month from now()::timestamptz at time zone 'Asia/Jakarta')::int],
