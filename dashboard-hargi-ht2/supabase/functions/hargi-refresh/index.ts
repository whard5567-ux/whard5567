...
function mapCeAbo(rows: Row[]) {
  ...
  return rows
    .filter((r) => {
      ...
      const isHargi = sub.includes("HARGI");
  ...
  try {
    ...
    const [ceRaw, ggnRaw, aboRaw, bushingRaw, meta] = await Promise.all([
      fetchSheetFiltered(CE_ABO, (letterOf) => {
        ...
        return `${sb} contains 'HARGI' and (upper(${st}) = 'OPEN' or upper(${st}) = 'CLOSE' or upper(${ka}) contains 'GOOD' or upper(${ka}) contains 'FAIR' or upper(${ka}) contains 'POOR' or upper(${ka}) contains 'CRITICAL')`;
      }),
