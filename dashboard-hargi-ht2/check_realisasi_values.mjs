...
async function run() {
  ...
  const tglRealisasiValues = new Set();

  for (let i = 2; i < parsed.data.length; i++) {
    const row = parsed.data[i];
    if (tglRealisasiIdx >= 0 && row[tglRealisasiIdx]) {
        tglRealisasiValues.add(row[tglRealisasiIdx]);
    }
  }
  console.log("Unique TANGGAL REALISASI values:", Array.from(tglRealisasiValues));
}
run();
